...
include("conn.php");
//数据库查询语句
$sql="select * from  question where deletes='1' ";
//数据库sql语句执行
$rs=mysql_query($sql);
if(!$rs){
...
//如果试题数量不为0
		...
		while($row=mysql_fetch_array($rs))
		{
			...
			//这是javascript伪地址的用法，confirm是一个方法
		   echo"<td ><a href=\"javascript:if(confirm('您确定要还原吗？')){location.href='huanyuan.php?num=".$row['num']."&subject=".$row['subject']."  ' }\"}>还原试题</a>   <a href=\"javascript:if(confirm('您确定要彻底删除吗？')){location.href='buhuanyuan.php?num=".$row['num']." ' }\"}>彻底删除</a></td>";
